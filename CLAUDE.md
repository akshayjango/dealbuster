...
4. Dedup is by product `id` AND `asin` (uppercased). The DO ledger is authoritative; KV `tg_posted_ids` is only an advisory mirror for the cron's cheap pre-check. Exception: when a deal is evicted at the 720 cap, `clearTgPostedForEvicted()` erases its ledger entries (DO first, then mirror) so a later re-add posts to the channel again as new. DO-clear failure aborts the mirror clear — a re-add is then silently skipped (missed post, never a duplicate).
