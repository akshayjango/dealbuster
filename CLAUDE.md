...
4. Dedup is by product `id` AND `asin` (uppercased). The DO ledger is authoritative; KV `tg_posted_ids` is only an advisory mirror for the cron's cheap pre-check. `queueForApproval()` also claims through the DO (`/posted/claim`) — never KV-only (KV read-modify-write races lost claims and re-DM'd the same deals 30+ times). `clearTgPostedForEvicted()` runs ONLY when a tombstone expires (14 days), never at eviction time — the feeds still list freshly evicted deals, and clearing at eviction re-DM'd every re-add (the July 2026 spam loop). DO-clear failure aborts the mirror clear — a re-add is then silently skipped (missed post, never a duplicate).
